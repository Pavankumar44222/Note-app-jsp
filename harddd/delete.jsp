...
<body>
    ...
    try{
        ...
        PreparedStatement preparedStatements = con.prepareStatement("select * from notes where id=?");
        preparedStatements.setString(1,id);
        out.print("after select * from table");
        ResultSet rs=preparedStatements.executeQuery();
        if(rs.next())
        {
            PreparedStatement create = con.prepareStatement("insert into recycle(username,title,content) values(?,?,?)");
